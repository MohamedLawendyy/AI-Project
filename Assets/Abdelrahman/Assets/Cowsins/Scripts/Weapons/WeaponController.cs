...
namespace cowsins
{
    public class WeaponController : MonoBehaviour
    {
        ...
        private void Hit(LayerMask layer, float damage, RaycastHit h, bool damageTarget)
        {
            ...
            // 1. Check for Headshot
            if (h.collider.gameObject.CompareTag("Critical"))
            {
                var damageable = CowsinsUtilities.GatherDamageableParent(h.collider.transform);
                if (damageable != null) // Check if script exists before damaging
                {
                    damageable.Damage(finalDamage * weapon.criticalDamageMultiplier, true);
                }
            }
            // 2. Check for BodyShot
            ...
            // 3. Check for Direct Hit (Component on the same object)
            else
            {
